...
void __pr_color(char code, const char *fmt, ...)
{
	...
	if (html_mode) {
		cs = HTML_COLOR_NORMAL;
		es = HTML_COLOR_RESET;
	}
	else {
		cs = TERM_COLOR_NORMAL;
		es = TERM_COLOR_RESET;
	}
...
static void __print_time_unit(int64_t delta_nsec, bool needs_sign)
{
	...
	char *html_color_units[] = {
		HTML_COLOR_NORMAL "us" HTML_COLOR_RESET,
...
{
	if (html_mode || out_color == COLOR_ON)
		return color;
	else
...
void print_diff_percent(uint64_t base_nsec, uint64_t pair_nsec)
{
	...
	if (html_mode) {
		sc = get_color(HTML_COLOR_NORMAL);
		ec = get_color(HTML_COLOR_RESET);
	}

	if (base_nsec == 0) {
		sc = get_color(TERM_COLOR_RED);
		if (html_mode)
			sc = get_color(HTML_COLOR_RED);
	...
	if (html_mode) {
		sc = percent > 30 ? HTML_COLOR_RED :
			percent > 3 ? HTML_COLOR_MAGENTA :
			percent < -30 ? HTML_COLOR_BLUE :
			percent < -3 ? HTML_COLOR_CYAN : HTML_COLOR_NORMAL;
		ec = HTML_COLOR_RESET;
	}
	else if (out_color == COLOR_ON) {
...
void print_diff_count(uint64_t base, uint64_t pair)
{
	...
	if (html_mode) {
		sc = get_color(html_diff_colors[sign_idx]);
		ec = get_color(HTML_COLOR_RESET);
	}
	else {
		sc = get_color(diff_colors[sign_idx]);
		ec = get_color(TERM_COLOR_RESET);
	}
